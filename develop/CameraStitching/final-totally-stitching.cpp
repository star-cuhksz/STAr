...
int main(int argc, char const* argv[]) {
  if (argc != 13) {
    printf(
        "Usage: ./stitching camera1 camera2 camera3 camera4 camera5 camera6 "
  ...
  t = clock();
  camera1 = imread(argv[1]);
  camera2 = imread(argv[2]);
  camera3 = imread(argv[3]);
  camera4 = imread(argv[4]);
  camera5 = imread(argv[5]);
  camera6 = imread(argv[6]);
  printf("reading images takes %f seconds\n",
         float(clock() - t) / CLOCKS_PER_SEC);

  t = clock();
  homo1to2 = readhomography(argv[7]);
  homo3to2 = readhomography(argv[8]);
  homo4to5 = readhomography(argv[9]);
  homo6to5 = readhomography(argv[10]);
  homo2to5 = readhomography(argv[11]);

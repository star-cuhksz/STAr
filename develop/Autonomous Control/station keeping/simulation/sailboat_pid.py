...
class sailboat:
    ...
    def sail_control(self):
        ...
        if angular_acc<0:
            ...
            # print(limit_sail,obtained_angle)
    ...
    def arrive(self,target_x,target_y,dT,dM):
        ...
        if distance_st<dM and distance_st>dT and math.cos(boat_to_target_angle-self.true_wind[1])>0:
            ...
            if math.cos(afa-(self.true_wind[1]+math.pi/2))>math.cos(afa-(self.true_wind[1]-math.pi/2)):
                self.goal_angle=math.pi/2+boat_to_target_angle
            else:
                self.goal_angle=-math.pi/2+boat_to_target_angle
        else:
            self.goal_angle=boat_to_target_angle

        #step 3 ##this is for the upwind case
        dead_angle=math.pi/4
        ##if not exceeding dead angle
        if math.cos(self.true_wind[1]-self.goal_angle)+math.cos(dead_angle)>0:
            self.desired_angle=self.goal_angle
            ###prepare for reaching the dead angle, determine which way is more convinient to tack
            if math.cos(self.goal_angle-(self.true_wind[1]+math.pi-dead_angle))>math.cos(self.goal_angle-(self.true_wind[1]+math.pi+dead_angle)):
                self.q=1
            else:
                self.q=-1
        else: 

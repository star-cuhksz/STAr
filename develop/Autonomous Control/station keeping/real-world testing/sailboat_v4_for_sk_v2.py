...
class sailboat:
    ...
    rudder=90,sail=0,jibsail=0,sample_time=0.1,acc_v=0,acc_u=0,acc_w=0,desired_acc=0):
    ####    all the units of angles are rad 
        ...
        self.acc_u=acc_u
        self.acc_w=acc_w
        self.target=[3,4]     ###the center of target area[x,y]   dM=10,which is the radius of the pre-arrived area ,dT=5,which is r of target area 
        self.desired_acc=desired_acc            
        self.obtained_angle=0
        ...
        self.maxrudder=math.pi/4.3  ##max angle of rudder
        self.maxsail=1.2
        self.true_wind=[5,3*math.pi/2]       ##[wind speed, direction]
        self.app_wind=[0,0]                 ##[wind speed, direction]
        self.sample_time=0.1               ##time for every single update
        self.ks=1               ###need to be adjusted ###
        self.aligned_p=0.2       ####need to be adjusted ##about 15 degrees of the mini sailboat
    ...
    def get_app_wind(self):
        ...
        # self.app_wind=[self.true_wind[0]*math.cos(self.true_wind[1]-self.heading_angle)-self.velocity[0],self.true_wind[0]*math.sin(self.true_wind[1]-self.heading_angle)-self.velocity[1]]
        self.app_wind=[self.true_wind[0]*math.cos(self.true_wind[1]-self.heading_angle)-abs(self.velocity[0]),self.true_wind[0]*math.sin(self.true_wind[1]-self.heading_angle)-abs(self.velocity[1])]
    ...
    def sail_control(self):
        ...
        angular_acc=-self.ks*(self.desired_acc-self.acc_v)*self.sign(self.sail)
        self.obtained_angle=max(angular_acc*self.sample_time+abs(self.obtained_angle),0)###the angle of sail
        ...
        #     ##################################
        #     # self.sail=-self.sign(self.app_wind[1])*max(self.obtained_angle,min(abs(optimal_sail),limit_sail))
        #     #############################
        #     self.sail=max(self.obtained_angle,min(abs(optimal_sail),limit_sail))
        # else:
        #     ### we put the sail to limit to slow down
        #     self.sail=min(pratical_sail_max,self.obtained_angle)

        if angular_acc<0:
            # print('accelerate!!!!',end='')
            optimal_sail=2.5*math.pi/4*(math.cos(self.true_wind[1]-self.desired_angle)+1)
            limit_sail=max(pratical_sail_max-self.aligned_p,0)
            ##################################
            # self.sail=-self.sign(self.app_wind[1])*max(self.obtained_angle,min(abs(optimal_sail),limit_sail))
            #############################
            self.sail=min(abs(optimal_sail),limit_sail)
            print('sail',self.sail)
        else:
            ### we put the sail to limit to slow down
            self.sail=min(pratical_sail_max,self.obtained_angle)
    ...
    def arrive(self,target_x,target_y,dT,dM):
        ...
        print('boat',boat_to_target_angle,end='')
        ...
        if distance_st<dT:
            ...
            if self.velocity[0]>0:
                ...
                self.sail=abs(math.pi-abs(self.app_wind[1]))
        ...
        ##add BOUNDARY
        if 1.5<self.location[0] and self.location[0]<4.5 and 2<self.location[1] and self.location[1]<8:
            print('...\n....\n...\n')
        else :
            if self.x_value==1 and self.y_value==1:
                self.desired_angle=0.6
            elif self.x_value==1 and self.y_value==-1:
                self.desired_angle=5.68
            elif self.x_value==-1 and self.y_value==-1:
                self.desired_angle=3.74
            elif self.x_value==-1 and self.y_value==1:
                self.desired_angle=2.54


        if self.desired_angle>math.pi*2:
            self.desired_angle-=math.pi*2
        if self.desired_angle<0:
            self.desired_angle+=math.pi*2
    def update_real(self,x,y,heading_angle,sample_time):
        self.velocity[0]=(x-self.location[0])*math.cos(self.choosen_angle)+(y-self.location[1])*math.sin(self.choosen_angle)/sample_time
        self.velocity[1]=(x-self.location[0])*-math.sin(self.choosen_angle)+(y-self.location[1])*math.cos(self.choosen_angle)/sample_time
        self.angular_velocity=(heading_angle-self.heading_angle)/sample_time
        self.location[0]=x
        self.location[1]=y
        self.heading_angle=heading_angle
    ...
    def updata_pos(self):
        ...
        g_s=self.p4*self.true_wind[0]*math.sin(self.sail-self.app_wind[1])

        self.acc_v=(g_s*math.sin(self.sail)-g_rv*self.p11*math.sin(self.rudder)-self.p2*self.velocity[0]*abs(self.velocity[0])
        +self.p1*pow(self.true_wind[0],2)*math.cos(self.true_wind[1]-self.heading_angle))/self.p9#原本为true_wind

        self.acc_u=-(g_ru*self.p11*math.cos(self.rudder)-self.p2*self.velocity[1]*abs(self.velocity[1])
        +self.p1*pow(self.true_wind[0],2)*math.sin(self.true_wind[1]-self.heading_angle))/self.p9
        # print(['heading',self.heading_angle,'velocity',self.velocity,'appwind',self.app_wind,'sail',self.sail,'rudder',self.rudder,'disire',self.desired_angle])
        self.acc_w=(g_s*(self.p6-self.p7*math.cos(self.sail))-g_rv*self.p8*math.cos(self.rudder)-self.p3*self.angular_velocity*self.velocity[0])/self.p10
    ...
    def to_next_moment(self):
        self.get_app_wind()
        self.update_real(self.location[0],self.location[1],self.heading_angle,self.sample_time)
        self.choose_angle()
        self.rudder_control()
        self.get_desired_acc(self.target[0],self.target[1])
        self.sail_control()
        self.arrive(self.target[0],self.target[1],0.8,1.5)
